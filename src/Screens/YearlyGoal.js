...
class YearlyGoal extends React.Component {
  ...
  componentDidMount() {
    ...
    axios
      .get(
        'https://intense-harbor-45607.herokuapp.com/get/image/' +
          this.props.user +
          '/2020'
      )
  ...
  _deleteImage = async () => {
    axios
      ...
      .then(resp => this.setState({ isDeleted: true }))
      .catch(err => console.log(err));
  };
  ...
  uploadImage = async uri => {
    ...
    var myfileName =
      'bg-' + this.props.user + '-' + this.state.selectedYear + '.' + 'jpg';
    this.setState({
      myfileName,
    });
  ...
  render() {
    ...
    if (this.state.transctions !== null) {
      ...
      for (let i = 0; i < this.state.transctions.length; i++) {
        if (
    ...
    let Image_Http_URL = '';
    // console.log("before",Image_Http_URL)
    Image_Http_URL = {
      uri: this.state.imgURL + '?v=' + new Date(),
    };
    // console.log("after",Image_Http_URL)
    ...
    return (
      <View style={{ backgroundColor: 'rgba(255,255,255,0.5)' }}>
        <ImageBackground
          ...
          source={this.state.isDeleted ? null : Image_Http_URL}>
